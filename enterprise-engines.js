...
(function(){
  ...
  window.RPBrainEnterprise={
    ...
    bind(container=document){container.querySelectorAll('.rp-open-person').forEach(b=>b.onclick=()=>personDetail(b.dataset.emp));container.querySelectorAll('.rp-open-action').forEach(b=>b.onclick=()=>actionDetail(b.dataset.id));container.querySelectorAll('.rp-open-knowledge').forEach(b=>b.onclick=()=>knowledgeArticleDetail(b.dataset.id));}
  };
  ...
  window.metlIntelligence=function(){const all=activePeople(),avg=all.length?Math.round(all.reduce((n,p)=>n+personMetrics(p).pct,0)/all.length):0;page('RP Brain','Powered by RP IA',`<div class="ai-hero"><div><span class="ai-badge">Powered by RP IA</span><h2>Readiness advisor</h2><p>Ask about employees, advancement, tasks, corrective actions, shifts, qualifications, or approved company knowledge.</p></div><div class="ai-score"><b>${avg}%</b><span>department readiness</span></div></div><div class="card full brain-ask-first"><h3>Ask RP Brain</h3><div class="ask-row"><input id="aiQuestion" placeholder="Example: What does Luis need to advance?"><button id="askAi" class="primary">Analyze</button></div><div class="quick-prompts"><button>What does Luis need to advance?</button><button>Show overdue corrective actions</button><button>Who can perform Die Move independently?</button><button>How do I perform a die move?</button></div><div id="aiAnswer" class="ai-answer">Ask in natural language. RP Brain keeps the current conversation context.</div></div><div class="grid"><div class="card"><h3>Competency Coach</h3><p>Positive, role-aware development guidance for each employee.</p><button class="secondary" id="openPersonnelCoach">Open personnel</button></div><div class="card"><h3>Knowledge Center</h3><p>${(state.knowledge||[]).filter(a=>a.status==='Approved').length} approved article${(state.knowledge||[]).filter(a=>a.status==='Approved').length===1?'':'s'} available to RP Brain.</p><button class="secondary" id="openKnowledgeFromBrain">Open Knowledge Center</button></div><div class="card full"><h3>Experience Engine</h3><p>${escHtml(ExperienceEngine.summary())}</p></div></div>`);const run=()=>{const q=$('#aiQuestion').value.trim();if(!q)return;const r=RPBrainEnterprise.answer(q);$('#aiAnswer').innerHTML=r.html;RPBrainEnterprise.bind($('#aiAnswer'));trackInterest(r.intent,3)};$('#askAi').onclick=run;$('#aiQuestion').onkeydown=e=>{if(e.key==='Enter')run()};$$('.quick-prompts button').forEach(b=>b.onclick=()=>{$('#aiQuestion').value=b.textContent;run()});$('#openPersonnelCoach').onclick=()=>navigate('personnel');$('#openKnowledgeFromBrain').onclick=()=>navigate('knowledge')};
